...
- `overview` —— 用一小段 markdown（几句话或几个要点）描述这个 PR 实际做了什么：按意图归类的主要改动。描述改动本身，而不是你的 findings。
- `manualReview` —— 需要人工自行重点审查的文件或区域，每条都附一个具体的 `reason`。用它来标注自动化评审无法完全验证的部分：判断性取舍、安全敏感面、依赖 diff 之外上下文的行为，或任何你确实没把握的地方。`file` 设为仓库相对路径，PR 级别的说明则设为 `null`。若没有什么需要人工特别关注的，空数组就是正确答案 —— 不要硬凑。
