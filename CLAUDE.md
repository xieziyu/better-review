...
8. **Parse findings + summary** — `engine/findings-watcher.ts` uses `chokidar` on `<workdir>/findings.json`. `engine/findings-parser.ts` validates entries against the zod schema in `src/shared/findings-schema.ts` and dedupes by `(file, line, title)`. New rows go through `db/findings.ts` and get broadcast on the SSE bus. In parallel, `engine/summary-watcher.ts` + `summary-parser.ts` watch `<workdir>/summary.json` (the agent's review overview + manual-review notes, schema in `src/shared/summary-schema.ts`); a successful parse is persisted via `sessions.setSummary` and broadcast as a `summary-generated` event. The summary is best-effort — a parse failure emits a non-fatal `error` event and leaves the session's summary null.
